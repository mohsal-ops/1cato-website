...
function SEOTip({ done, text, action, href }: { done: boolean; text: string; action?: string; href?: string }) {
  return (
    <div className="flex items-start gap-3 py-3 border-b border-stone-50 last:border-0">
      ...
      {!done && href && action && (
        <Link href={href} className="text-xs font-semibold text-[#ea580c] hover:underline shrink-0">{action} →</Link>
      )}
    </div>
  );
}
      ...
      >
        ...
        <div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
            <Link href="/admin/Blog" className="group bg-white rounded-2xl border border-stone-200 shadow-sm p-5 hover:border-[#ea580c] hover:shadow-md transition-all">
              <div className="w-9 h-9 rounded-xl bg-orange-50 text-[#ea580c] flex items-center justify-center mb-3 group-hover:scale-110 transition-transform"><PenLine size={18} /></div>
        ...
        <div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            <StatCard label="Blog Posts" value={data.totalPosts} sub={data.latestPost ? `Last: "${data.latestPost.title.slice(0, 22)}..."` : "No posts yet"} icon={<BookOpen size={16} />} accent="#ea580c" />
        ...
        <div className="grid lg:grid-cols-2 gap-6">
          <div className="bg-white rounded-2xl border border-stone-200 shadow-sm p-6">
            ...
            <SEOTip done={data.totalLocations >= 1} text="Location added (helps local SEO in Brooklyn)" action="Add location" href="/admin/places" />
            <div className="mt-4 bg-stone-50 rounded-xl p-3 flex gap-2">
              <Zap size={14} className="text-[#ea580c] shrink-0 mt-0.5" />
          ...
          <div className="bg-white rounded-2xl border border-stone-200 shadow-sm p-6">
            <div className="flex items-center justify-between mb-4">
              <div className="flex items-center gap-3">
                <div className="w-9 h-9 rounded-xl bg-orange-50 text-[#ea580c] flex items-center justify-center"><TrendingUp size={16} /></div>
                <div>
              ...
              <Link href="/admin/Blog" className="text-xs font-semibold text-[#ea580c] hover:underline flex items-center gap-1">Manage <ArrowRight size={12} /></Link>
            </div>
          ...
          <div className="bg-white rounded-3xl shadow-2xl p-8 max-w-sm w-full mx-4 flex flex-col items-center gap-4 text-center">
            ...
            <div className="w-full">
              <div className="h-2 w-full bg-stone-100 rounded-full overflow-hidden">
                <div className="h-full w-[80%] bg-[#ea580c] rounded-full animate-pulse" />
              </div>
              <p className="text-xs text-stone-400 mt-2">Progress: 80% complete</p>
            </div>
            <p className="text-xs text-stone-400">
              Questions? Email{" "}
              <a href="mailto:popdeveloper54@gmail.com" className="text-[#ea580c] hover:underline font-medium">
